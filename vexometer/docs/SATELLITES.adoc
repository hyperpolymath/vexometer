...
NOTE: The `.scm` state-file format shown below is retired — the estate
migrated machine-readable metadata to A2ML under `.machine_readable/6a2/`
(see this repository's `ECOSYSTEM.a2ml` / `META.a2ml` / `STATE.a2ml`). The
template is kept for its *content* shape; a new satellite should emit the
equivalent A2ML.
...
NOTE: `just efficacy-report` is not yet implemented, and this v1
signature is superseded by the `--frontier` workflow in
link:EFFICACY-PROTOCOL.adoc[EFFICACY-PROTOCOL.adoc], which records
collateral metrics and capability. The command below documents the
intended v1 shape only.
...
* link:EFFICACY-PROTOCOL.adoc[Efficacy Protocol] - Acceptance rule and frontier records extending vexometer-efficacy-v1
* link:../../.machine_readable/6a2/ECOSYSTEM.a2ml[Ecosystem Map] - Satellite relationships
* link:../../.machine_readable/6a2/META.a2ml[Project Metadata] - ISA meta-information
